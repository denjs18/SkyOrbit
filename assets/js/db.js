// ============================================
// COUCHE DE DONNÉES - Horizon Libre
// ============================================
...
(function () {
    ...
    const DEMO_KEY = 'horizonlibre_demo_db';
    ...
    function demoSeed() {
        ...
        return {
            profiles: [
                { id: 'u-admin', email: 'admin@horizon-libre.fr', password: 'admin123', full_name: 'Alice Bureau', phone: '06 00 00 00 01', role: 'admin', license_number: 'ULM-12345', license_expiry: '2027-03-01', medical_expiry: '2026-09-15', qualifications: 'Multiaxe, Instructeur', active: true },
                { id: 'u-inst', email: 'instructeur@horizon-libre.fr', password: 'inst123', full_name: 'Jean Moreau', phone: '06 00 00 00 02', role: 'instructor', license_number: 'ULM-23456', license_expiry: '2026-12-01', medical_expiry: '2026-07-20', qualifications: 'Multiaxe, Pendulaire, Instructeur', active: true },
                { id: 'u-pilot', email: 'pilote@horizon-libre.fr', password: 'pilot123', full_name: 'Marc Leroy', phone: '06 00 00 00 03', role: 'pilot', license_number: 'ULM-34567', license_expiry: '2026-08-10', medical_expiry: '2026-06-25', qualifications: 'Multiaxe', active: true },
                { id: 'u-sophie', email: 'sophie.martin@example.fr', password: 'demo123', full_name: 'Sophie Martin', phone: '06 00 00 00 04', role: 'member', license_number: 'ULM-45678', license_expiry: '2027-01-15', medical_expiry: '2027-02-01', qualifications: 'Élève pilote', active: true }
            ],
            machines: [
                { id: 'm-1', registration: 'F-JABC', model: 'Skyranger Nynja', type: 'multiaxe', hours_total: 1240.5, maintenance_interval: 50, last_maintenance_hours: 1220, notes: '', active: true },
                { id: 'm-2', registration: 'F-JDEF', model: 'Air Création Tanarg', type: 'pendulaire', hours_total: 860, maintenance_interval: 50, last_maintenance_hours: 855, notes: 'Radio neuve', active: true },
                { id: 'm-3', registration: 'F-JGHI', model: 'Magni M-16', type: 'autogire', hours_total: 432, maintenance_interval: 100, last_maintenance_hours: 400, notes: '', active: true }
            ],
            maintenance_logs: [
            ...
            reservations: [
                { id: 'r-1', machine_id: 'm-1', member_id: 'u-pilot', instructor_id: null, start_at: d(1, 9), end_at: d(1, 11), flight_type: 'libre', status: 'confirmed', actual_hours: null, remarks: '' },
                { id: 'r-2', machine_id: 'm-2', member_id: 'u-sophie', instructor_id: 'u-inst', start_at: d(1, 14), end_at: d(1, 16), flight_type: 'formation', status: 'pending', actual_hours: null, remarks: 'Premier vol solo en vue' },
                { id: 'r-3', machine_id: 'm-3', member_id: 'u-inst', instructor_id: null, start_at: d(3, 10), end_at: d(3, 12), flight_type: 'bapteme', status: 'confirmed', actual_hours: null, remarks: 'Baptême offert anniversaire' },
                { id: 'r-4', machine_id: 'm-1', member_id: 'u-pilot', instructor_id: null, start_at: d(-7, 9), end_at: d(-7, 10, 30), flight_type: 'libre', status: 'completed', actual_hours: 1.5, remarks: '' }
            ],
            cotisations: [
                { id: 'c-1', member_id: 'u-admin', year: 2026, amount: 250, status: 'paid', paid_at: '2026-01-10' },
                { id: 'c-2', member_id: 'u-inst', year: 2026, amount: 250, status: 'paid', paid_at: '2026-01-15' },
                { id: 'c-3', member_id: 'u-pilot', year: 2026, amount: 250, status: 'paid', paid_at: '2026-02-01' },
                { id: 'c-4', member_id: 'u-sophie', year: 2026, amount: 250, status: 'unpaid', paid_at: null }
            ],
            baptemes: [
                { id: 'b-1', slot_at: d(3, 10), formula: 'decouverte', price: 60, customer_name: 'Paul Petit', customer_email: 'paul.petit@example.fr', customer_phone: '06 11 22 33 44', pilot_id: 'u-inst', machine_id: 'm-3', status: 'paid', payment_ref: 'DEMO-001', notes: '' },
                { id: 'b-2', slot_at: d(8, 15), formula: 'sensation', price: 90, customer_name: 'Emma Grand', customer_email: 'emma.grand@example.fr', customer_phone: '06 55 66 77 88', pilot_id: null, machine_id: null, status: 'pending', payment_ref: null, notes: 'Demande créneau fin de journée' }
            ]
        };
    }
    ...
    const DB = {
        ...
        async listMembers() {
            ...
            return demoLoad().profiles.map(stripPassword).sort((a, b) => a.full_name.localeCompare(b.full_name));
        },

        async createMember(fields) {
            if (sb) {
                const { data, error } = await sb.from('profiles').insert(fields).select().single();
                sbThrow(error);
                return data;
            }
            const db = demoLoad();
            const p = { id: demoId('u'), active: true, role: 'member', password: 'demo123', ...fields };
            db.profiles.push(p);
            demoSave(db);
            return stripPassword(p);
        },
        ...
        async listCotisations(year) {
            ...
            const all = demoLoad().cotisations;
        ...
        async setCotisation(fields) {
            if (sb) {
                const { data, error } = await sb.from('cotisations')
                    .upsert(fields, { onConflict: 'member_id,year' }).select().single();
                sbThrow(error);
                return data;
            }
            const db = demoLoad();
        ...
        async listMachines() {
            ...
            return demoLoad().machines.slice().sort((a, b) => a.registration.localeCompare(b.registration));
        },

        async createMachine(fields) {
            if (sb) {
                const { data, error } = await sb.from('machines').insert(fields).select().single();
                sbThrow(error);
                return data;
            }
            const db = demoLoad();
            const m = { id: demoId('m'), hours_total: 0, last_maintenance_hours: 0, maintenance_interval: 50, active: true, notes: '', ...fields };
            db.machines.push(m);
            demoSave(db);
            return m;
        },
        ...
        async createReservation(fields) {
            ...
            if (sb) {
                const { data, error } = await sb.from('reservations')
                    .insert({ status: 'pending', ...fields }).select(RESERVATION_SELECT).single();
                sbThrow(error);
                return flattenSbReservation(data);
            }
            const db = demoLoad();
            const r = { id: demoId('r'), status: 'pending', actual_hours: null, remarks: '', instructor_id: null, ...fields };
        ...
        async listBaptemes() {
            ...
            return demoLoad().baptemes.slice().sort((a, b) => a.slot_at.localeCompare(b.slot_at));
        },

        // Appelé depuis la page publique : autorisé sans connexion.
        async createBapteme(fields) {
            if (sb) {
                const { data, error } = await sb.from('baptemes').insert({ status: 'pending', ...fields }).select().single();
                sbThrow(error);
                return data;
            }
            const db = demoLoad();
            const b = { id: demoId('b'), status: 'pending', pilot_id: null, machine_id: null, payment_ref: null, notes: '', ...fields };
            db.baptemes.push(b);
            demoSave(db);
            return b;
        },
